...
function capitalA(s) {
    if ((/\b[A-Z]{1}[a-z\s]+A\b/g).test(s) === true) {
        return true;
    } else {
        return false;
    }
...
function ioEmail(email) {
    if ((/^\w+@\w+.io\b/ig).test(email) === true) {
        return true;
    } else {
        return false;
    }
...
function imagesSearcher(text) {
    let arr = [];
    let imgExtention = /\w+.(jpeg|jpg|png)/ig
    if (imgExtention.test(text) === true) {
        arr = (text.match(imgExtention))
        return arr
    } else {
        return arr
    }
